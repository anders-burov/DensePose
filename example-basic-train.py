...
if __name__ == '__main__':
    ...
    optimizer = optim.Adam(net.parameters(), amsgrad=True) #defaults: lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0   https://pytorch.org/docs/stable/optim.html?highlight=optim#torch.optim.Adam
    for epoch in range(epochs):
        if not overfit:
            pids_shuffled = np.random.permutation(dataload.train_persons_cnt)
        else:
            print('OVERFIT!' * 100)
            pids_shuffled = np.array([0, 1, 2])
            batch_size = 3
        ...
        for bidx in range(n_batches):
            ...
            for pid in batch_pids:
                ...
                #
                # TODO compute intersect. COuld be here or below.
                #
                ...
                batch_of_IUV_stacks.append( (S1, S2) )
            #print('Done compute IUV stacks')
            input1s = []; input2s = []; targets = []
            for person in range(len(batch_of_IUV_stacks)):
                #
                # TODO compute intersect. COuld be here or above.
                #
                # Same person pair:
                input1s.append(batch_of_IUV_stacks[person][0])
                input2s.append(batch_of_IUV_stacks[person][1])
                targets.append(1)
                # Diff persons pair:
                persons = list(range(len(batch_of_IUV_stacks)))
                persons.remove(person)
                another_person = random.choice(persons)
                assert(person != another_person) # TODO remove.
                input1s.append(batch_of_IUV_stacks[person][random.randint(0,1)])
                input2s.append(batch_of_IUV_stacks[another_person][random.randint(0,1)])
                targets.append(0)
            assert(len(input1s) > 0)
            assert( len(input1s) == len(input2s) and len(input2s) == len(targets) )
            #print('len ', len(input1s))
            # torch stack input1s, input2s, targets.
            #print('stack')
            ...
            # x0, x1, y each having shape[0] of 2*batch_size
